  ...
  constructor(props) {
    ...
    if (this.botconfig.Token === "")
      return new TypeError(
        "botconfig.js boş bırakılmaz. Lütfen hiçbir şeyin boş olmadığından emin olun, aksi takdirde bot düzgün çalışmayacaktır.."
      );
    ...
    this.Manager = new Manager({
      ...
      .on("trackStart", async (player, track) => {
        ...
        let TrackStartedEmbed = new MessageEmbed()
          .setAuthor(`Oynatılan 🎵`, this.botconfig.IconURL)
          .setThumbnail(player.queue.current.displayThumbnail())
          .setDescription(`[${track.title}](${track.uri})`)
          .addField("Oynatan Kişi", `${track.requester}`, true)
          .addField(
            "Süre",
      ...
      .on("queueEnd", (player) => {
        let QueueEmbed = new MessageEmbed()
          .setAuthor("Sıranın sonuna geldik", this.botconfig.IconURL)
          .setColor(this.botconfig.EmbedColor)
          .setTimestamp();
        ...
        files.forEach((file) => {
          ...
          if (!cmd.name || !cmd.description || !cmd.run)
            return this.log(
              "Komut yüklenemiyor: " +
                file.split(".")[0] +
                ", Sebep: run/name/desciption dosya yok"
            );
  ...
  build() {
    ...
    if (this.botconfig.ExpressServer) {
      this.http.listen(process.env.PORT || this.botconfig.Port, () =>
        this.log("Web Sunucusu başlatıldı")
      );
    }
  }
